...
- 已新增 `src/api/merchant-food.ts`、`src/api/types/merchant-food.ts` 和编译期 API 契约。
- 已新增 `src/store/merchant-food.ts`，统一管理门店列表、当前 `storeId` 和门店 profile。
- 登录、注册、当前商家资料接口已经接入，见 `src/api/login.ts`。
- 请求拦截器会自动拼接服务器地址并携带 `Authorization: Bearer <token>`，见 `src/http/interceptor.ts`。
- 门店资料、营业状态、名称/地址审核、品类、入口图、电话、资质、相册、商品、订单、售后和对账页面均已接入 `/merchant/food/**`。
- 图片上传复用现有 `/upload`，上传拦截器会自动附加商家 Bearer Token；上传成功后再保存 merchant.food 业务记录。
- 前端功能分支：`feature/merchant-food-api-integration`。
- 验证结果：本次文件 ESLint 通过、`pnpm type-check` 通过、`pnpm build:h5` 通过；H5 无登录态路由回登录页且控制台无错误。
...
1. 图片上传已复用现有 `/upload`，仍需使用真实商家账号验证测试环境对该端点的权限和响应字段。
...
11. `MerchantFoodOrderVO` 的 Mapper 查询了 `orderTime`，但 Java VO 没有该字段；订单列表目前只能显示 `--`。
12. `MerchantFoodBillVO` 没有账单/结算时间字段；账单月份可筛选，但单条账单日期目前只能显示 `--`。
13. 本轮浏览器没有商家登录态，因此写操作尚未在真实账号下完成端到端验证。

## 9. 下一位 AI 的建议起始任务

前端现有接口对接已经完成。后续优先做真实账号联调和后端契约补强：

1. 使用具有 15 个 `merchant:food:*` 权限的商家账号登录 H5 或微信小程序。
2. 逐页验证 profile、营业状态、电话、资质和图片上传写操作。
3. 为相册准备审核通过图片，再验证商品创建、编辑和上下架。
4. 验证 19 条到店订单的列表、详情和联系方式。
5. 补充团购订单、退款、结算测试数据后验证审批和账单页面。
6. 后端为订单/账单 VO 增加时间字段后，同步替换页面中的 `--`。
7. 外卖页面完成后另开阶段，不在当前模块中提前调用 `KeloopClient`。
...
当前进度：现有页面接口对接已完成，待真实商家账号联调、补充团购测试数据，并补齐订单/账单时间字段。
